...
public class CommandController {
	...
	/**
	 * Retrieve a list of (all) devices and their command offerings. Throws
	 * ServiceException (HTTP 503) for unanticipated or unknown issues
	 * encountered.
	 * 
	 * @param host
	 *            - provided by the Spring runtime via the request header, the
	 *            host address.
	 * @return List of CommandResponse (containing the devices and their
	 *         commands)
	 */
	@RequestMapping(method = RequestMethod.GET)
	public List<CommandResponse> devices(@RequestHeader("host") String host) {
		try {
			return deviceClient.devices().stream().map(d -> new CommandResponse(d, host)).collect(Collectors.toList());
		} catch (Exception e) {
	...
	/**
	 * Retrieve a device (by database generated id) and its command offerings.
	 * Throws ServiceException (HTTP 503) for unanticipated or unknown issues
	 * encountered. Throws NotFoundException (HTTP 404) if no device exists by
	 * the id provided.
	 * 
	 * @param id
	 *            - the database generated id for the device
	 * @param host
	 *            - provided by the Spring runtime via the request header, the
	 *            host address.
	...
	public CommandResponse device(@PathVariable String id, @RequestHeader("host") String host) {
		...
		} catch (javax.ws.rs.NotFoundException eNF) {
	...
	/**
	 * Retrieve a device (by name) and its command offerings. Throws
	 * ServiceException (HTTP 503) for unanticipated or unknown issues
	 * encountered. Throws NotFoundException (HTTP 404) if no device exists by
	 * the name provided.
	 * 
	...
	public CommandResponse deviceByName(@PathVariable String name, @RequestHeader("host") String host) {
		try {
	...
	/**
	 * Issue the put command referenced by the command id to the device/sensor
	 * (also referenced by database generated id) it is associated to via the
	 * device service. ServiceException (HTTP 503) for unanticipated or unknown
	 * issues encountered. Throws NotFoundException (HTTP 404) if no device
	 * exists by the id provided. Throws LockedException (HTTP 423) if the
	 * device is locked (admin state).
	 * 
	 * @param id
	 *            - the database generated id for the device to receive the put
	 *            command request
	 * @param commandid
	 *            - the id (database generated id) of the command to issue to
	 *            the device
	 * @param host
	 *            - provided by the Spring runtime via the request header, the
	 *            host address.
	 * @param body
	 *            - JSON data to send with the command request
	...
	public ResponseEntity<String> put(@PathVariable String id, @PathVariable String commandid,
		...
		try {
			...
			if (device.getAdminState() == AdminState.locked) {
				...
				throw new LockedException("Device " + device.getName() + " is in admin locked state");
			}
			if (device.getOperatingState() == OperatingState.disabled) {
				logger.info("Put request to device:  " + device.getName() + " blocked because it is disable");
				throw new LockedException("Device " + device.getName() + " is in disabled op state");
			}
	...
	/**
	 * Issue the get command referenced by the command id to the device/sensor
	 * (also referenced by database generated id) it is associated to via the
	 * device service. ServiceException (HTTP 503) for unanticipated or unknown
	 * issues encountered. Throws NotFoundException (HTTP 404) if no device
	 * exists by the id provided. Throws LockedException (HTTP 423) if the
	...
	/**
	 * Set the op state of the device (as referenced by the database generated
	 * id of the device) to the state provided (either enabled or disabled).
	 * ServiceException (HTTP 503) for unanticipated or unknown issues
	 * encountered. Throws NotFoundException (HTTP 404) if no device exists by
	 * the id provided.
	 * 
	 ...
	 * 
	 * @param opState
	 *            - either enabled or disabled as a String
	...
	public boolean putOpState(@PathVariable String id, @PathVariable String opState) {
		try {
			...
			logger.info("Requesting op state for device: " + id + " be set to " + opState);
			return true;
		} catch (javax.ws.rs.NotFoundException eNF) {
			throw new NotFoundException(Device.class.toString(), id);
		} catch (Exception e) {
	...
	/**
	 * Set the op state of the device (by name of the device) to the state
	 * provided (either enabled or disabled). ServiceException (HTTP 503) for
	 * unanticipated or unknown issues encountered. Throws NotFoundException
	...
	public boolean putOpStateByName(@PathVariable String name, @PathVariable String opState) {
		try {
			...
			logger.info("Requesting op state for device: " + name + " be set to " + opState);
			return true;
		} catch (javax.ws.rs.NotFoundException eNF) {
			throw new NotFoundException(Device.class.toString(), name);
		} catch (Exception e) {
	...
	/**
	 * Set the admin state of the device (as referenced by the database
	 * generated id of the device) to the state provided (either locked or
	 * unlocked). ServiceException (HTTP 503) for unanticipated or unknown
	 * issues encountered. Throws NotFoundException (HTTP 404) if no device
	 * exists by the id provided.
	 * 
	 * @param id
	 *            - the database generated id for the device to receive the put
	 *            command request
	 * 
	 * @param adminState
	 *            - either locked or unlocked as a String
	 * @return - boolean indicating the successful
	 */
	...
	public boolean putAdminState(@PathVariable String id, @PathVariable String adminState) {
		try {
			...
			logger.info("Requesting admin state for device: " + id + " be set to " + adminState);
			return true;
		} catch (javax.ws.rs.NotFoundException eNF) {
			throw new NotFoundException(Device.class.toString(), id);
		} catch (Exception e) {
	...
	/**
	 * Set the admin state of the device (by name of the device) to the state
	 * provided (either locked or unlocked). ServiceException (HTTP 503) for
	 * unanticipated or unknown issues encountered. Throws NotFoundException
	 * (HTTP 404) if no device exists by the name provided.
	 * 
	 * @param name
	...
	public boolean putAdminStateByName(@PathVariable String name, @PathVariable String adminState) {
		try {
			...
			logger.info("Requesting admin state for device: " + name + " be set to " + adminState);
			return true;
		} catch (javax.ws.rs.NotFoundException eNF) {
			throw new NotFoundException(Device.class.toString(), name);
		} catch (Exception e) {
	...
	private String getUrl(Device device, String deviceId, Command command, boolean isGet) {
		...
		if (service != null && service.getAddressable() != null) {
			...
			if (Protocol.HTTP == service.getAddressable().getProtocol()) {
				StringBuilder builder = new StringBuilder(service.getAddressable().getProtocol().toString());
				builder.append("://");
	...
	private ResponseEntity<String> issueCommand(String url, String body, boolean isPut) throws IOException {
