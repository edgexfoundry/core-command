...
public interface CommandResponseData {
	...
	static void checkTestData(CommandResponse response) {
		...
		assertEquals("Command response device name does not match expected", TEST_NAME, response.getDevice().getName());
		assertArrayEquals("Command response device labels does not match expected", TEST_LABELS,
				response.getDevice().getLabels());
		assertEquals("Command response device last connected does not match expected", TEST_LAST_CONNECTED,
				response.getDevice().getLastConnected());
		assertEquals("Command response device last reported does not match expected", TEST_LAST_REPORTED,
				response.getDevice().getLastReported());
